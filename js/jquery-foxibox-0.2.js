 ...
 */
 (function($) {
  $.fn.foxibox = function(settings) {
    ...
    var foxibox_objects = '<div id="foxibox_overlay"></div><div id="foxibox_loader"></div><div id="foxibox_container"><img id="foxibox_image" src="" draggable="false"/><div id="foxibox_details"><div id="foxibox_title"></div><a href="#" id="close"></a><a href="#" id="scale"></a><div id="foxibox_nav"><a href="#" id="next"></a><a href="#" id="prev"></a><div></div></div></div></div>';
    ...
    function showImage(element){
      ...
      preload_img.onload = function(){
        ...
        $('#foxibox_container').animate({'width':container_width, 'height':container_height, 'left':container_left, 'top':container_top}, settings.speed, function(){
          ...
          $('#foxibox_details').fadeIn(settings.speed);
          if(rescaled){
	...
	var _move=false;
		var _x,_y;
		var _xd,_yd;
	    $("#foxibox_container").click(function(){
		}).mousedown(function(e){
		
		_move=true;
		_x=e.pageX-parseInt($("#foxibox_container").css("left"));
		_y=e.pageY-parseInt($("#foxibox_container").css("top"));
		_xd=e.pageX-parseInt($("#foxibox_details").css("left"));
		_yd=e.pageY-parseInt($("#foxibox_details").css("top"));
		return false;
	    });
		...
		if(_move){
		    ...
		    var xd=e.pageX-_xd;
		    var yd=e.pageY-_yd;
		    $("#foxibox_container").css({'top':y,'left':x});
		    $("#foxibox_details").css({'top':yd,'left':xd});
		}
	    }).mouseup(function(){
	    _move=false;
	  });
    ...
    //added by leedow
    function changeSize(){
    	$('#foxibox_image').mousewheel(function(event, delta) {
    		if(!lee_flag){alert();return false;}
		var lee_container_w = parseInt($('#foxibox_container').css('width'));
		var lee_container_h = parseInt($('#foxibox_container').css('height'));
		var lee_container_l = parseInt($('#foxibox_container').css('left'));
		var lee_container_t = parseInt($('#foxibox_container').css('top'));
		
		var lee_img_w = parseInt($('#foxibox_image').css('width'));
		var lee_img_h = parseInt($('#foxibox_image').css('height'));
		var lee_img_l = parseInt($('#foxibox_image').css('left'));
		var lee_img_t = parseInt($('#foxibox_image').css('top'));
		
		var lee_detail_w = parseInt($('#foxibox_details').css('width'));
		var lee_detail_l = parseInt($('#foxibox_details').css('left'));
		var lee_detail_t = parseInt($('#foxibox_details').css('top'));
		
		var www,hhh;
		
		if(delta > 0){
			www=100;
			hhh=100*(lee_img_h/lee_img_w);
		} else {
			if(lee_img_w <= 300){
				www=0;
				hhh=0;
		...
		lee_container_w += www;
		lee_container_h += hhh;
		lee_container_l -= Math.round(www/2);
		lee_container_t -= Math.round(hhh/2);
		
		lee_img_w += www;
		lee_img_h += hhh;
		lee_img_l -= Math.round(www/2);
		lee_img_t -= Math.round(hhh/2);
		
		lee_detail_w += www;
		lee_detail_l -= Math.round(www/2);

		//change size
		$('#foxibox_container').css({'width':lee_container_w, 'height':lee_container_h});
		$('#foxibox_image').css({'width':lee_img_w, 'height':lee_img_h});
		$('#foxibox_details').css({'width':lee_detail_w});
		
		//change position
		$('#foxibox_container').css({'top':lee_container_t, 'left':lee_container_l});
		$('#foxibox_image').css({'top':lee_img_t, 'left':lee_img_l});
		$('#foxibox_details').css({'top':lee_img_t + lee_img_h, 'left':lee_detail_l});
		lee_flag = true;
		return false;
        });
    }
